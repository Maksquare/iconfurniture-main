    ...
    <>
      ...
      <header className="fixed top-5 inset-x-0 z-50 flex flex-col items-center px-4 sm:px-6 pointer-events-none">
        <nav
          className={`pointer-events-auto w-full max-w-5xl rounded-full px-6 py-3 transition-all duration-500 flex items-center justify-between shadow-lg backdrop-blur-xl ${
            isDark
              ? 'bg-[#151412]/85 border border-white/10 text-stone-200 shadow-black/40'
              : 'bg-white/85 border border-stone-200/80 text-stone-800 shadow-stone-900/5'
          }`}
        >
          {/* Brand & Left Nav */}
          <div className="flex items-center gap-7">
            ...
            >
              <span className="w-2.5 h-2.5 rounded-full bg-[#d4a373] inline-block animate-pulse shadow-[0_0_10px_#d4a373]" />
              <span>ICON<span className="text-[#c28e58] font-normal">FURNITURE</span></span>
            </Link>
              ...
              {navLinks.map((item) => (
                <Link
                  ...
                  className={`transition-colors duration-200 py-1 ${
                    isDark
                      ? 'text-stone-400 hover:text-[#d4a373]'
          ...
          <div className="flex items-center gap-3 text-[13px]">
            ...
            <button
              ...
              className={`group flex items-center gap-2 pl-3 pr-3.5 py-2 rounded-full border transition-all duration-300 ${
                isDark
                  ? 'border-white/15 text-stone-300 hover:border-[#d4a373]/40 hover:bg-white/5'
                  : 'border-stone-200/80 text-stone-600 hover:border-[#d4a373] hover:bg-stone-50'
              }`}
            >
            ...
            <Link
              ...
              className={`hidden sm:inline-flex items-center gap-1.5 px-5 py-2 rounded-full font-medium transition-all duration-300 ${
                isDark
                  ? 'bg-white text-stone-950 hover:bg-[#d4a373] hover:text-stone-950 hover:shadow-[0_0_16px_rgba(212,163,115,0.4)]'
                  : 'bg-stone-950 text-white hover:bg-[#9a6b43]'
              }`}
            >
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-4 text-sm font-medium">
              ...
              >
                <Search className="w-4 h-4 text-[#9a6b43]" />
              ...
              {navLinks.map((item) => (
                <Link
                  ...
                  className="text-left py-1 hover:text-[#d4a373] transition-colors"
                >
                  {item.name}
                </Link>
              ))}
              ...
              <Link
                href="/shop"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-3 bg-[#9a6b43] text-white rounded-2xl font-semibold flex items-center justify-center gap-2"
              >
      ...
      {searchOpen && (
        ...
        >
          {/* Backdrop */}
          <div
            className="absolute inset-0 bg-[#0e0d0c]/85 backdrop-blur-xl"
            onClick={closeSearch}
          />
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="relative">
              <div className="flex items-center gap-3 bg-white rounded-2xl px-5 py-4 shadow-2xl border border-stone-200/50">
                <Search className="w-5 h-5 text-[#9a6b43] shrink-0" />
                <input
                  ref={inputRef}
                  type="text"
                  value={searchQuery}
                ...
                <button
                  type="submit"
                  className="shrink-0 px-4 py-2 bg-[#1C1917] text-white text-xs font-semibold rounded-xl hover:bg-[#9a6b43] transition-colors"
                >
                  Search
                </button>
              </div>
            ...
            <div className="mt-3 bg-white/95 backdrop-blur-sm rounded-2xl border border-stone-200/50 shadow-2xl overflow-hidden">
              ...
              <div className="p-4 border-b border-stone-100">
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles className="w-3.5 h-3.5 text-[#9a6b43]" />
                ...
                <div className="flex flex-wrap gap-2">
                  {filteredSuggestions.map((term) => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-stone-50 hover:bg-[#d4a373]/15 border border-stone-200/80 hover:border-[#d4a373]/40 text-xs text-stone-700 hover:text-[#7a5230] transition-all"
                    >
                        ...
                        <>
                          {term.split(new RegExp(`(${searchQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi')).map((part, i) =>
                            part.toLowerCase() === searchQuery.toLowerCase()
                              ? <mark key={i} className="bg-[#d4a373]/30 text-[#7a5230] font-semibold not-italic rounded">{part}</mark>
                  ...
                  {QUICK_LINKS.map((link) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <ArrowRight className="w-3.5 h-3.5 text-stone-400 group-hover:text-[#9a6b43] transition-colors" />
                        <span className="text-sm text-stone-700 group-hover:text-stone-900 transition-colors">{link.label}</span>
                      </div>
                      <span className="text-[10px] uppercase tracking-wider font-bold text-stone-400 group-hover:text-[#9a6b43] transition-colors">
                        {link.tag}
                      </span>
                    </Link>
                  ))}
