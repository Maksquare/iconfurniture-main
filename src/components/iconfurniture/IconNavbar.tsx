              ...
              {navLinks.map((item) => (
                <Link
                  ...
                  className={`transition-colors duration-200 py-1 ${
                    isDark
                      ? 'text-stone-400 hover:text-[#859F3C]'
                      : 'text-stone-600 hover:text-[#859F3C]'
                  }`}
                >
                  {item.name}
                </Link>
          ...
          <div className="flex items-center gap-3 text-[13px]">
            ...
            <button
              ...
              className={`group flex items-center gap-2 pl-3 pr-3.5 py-2 rounded-full border transition-all duration-300 ${
                isDark
                  ? 'border-white/15 text-stone-300 hover:border-[#859F3C]/40 hover:bg-white/5'
                  : 'border-stone-200/80 text-stone-600 hover:border-[#859F3C] hover:bg-stone-50'
              }`}
            >
            ...
            <Link
              ...
              className={`hidden sm:inline-flex items-center gap-1.5 px-5 py-2 rounded-full font-medium transition-all duration-300 ${
                isDark
                  ? 'bg-white text-[#1A1A1A] hover:bg-[#859F3C] hover:text-white hover:shadow-[0_0_16px_rgba(133,159,60,0.4)]'
                  : 'bg-[#1A1A1A] text-white hover:bg-[#859F3C]'
              }`}
            >
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-4 text-sm font-medium">
              ...
              >
                <Search className="w-4 h-4 text-[#859F3C]" />
              ...
              {navLinks.map((item) => (
                <Link
                  ...
                  className="text-left py-1 hover:text-[#859F3C] transition-colors"
                >
                  {item.name}
                </Link>
              ))}
              ...
              <Link
                href="/shop"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-3 bg-[#859F3C] hover:bg-[#738b32] text-white rounded-2xl font-semibold flex items-center justify-center gap-2 transition-colors"
              >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="relative">
              <div className="flex items-center gap-3 bg-white rounded-2xl px-5 py-4 shadow-2xl border border-stone-200/50">
                <Search className="w-5 h-5 text-[#859F3C] shrink-0" />
                <input
                  ref={inputRef}
                  type="text"
                  value={searchQuery}
                ...
                <button
                  type="submit"
                  className="shrink-0 px-4 py-2 bg-[#1A1A1A] hover:bg-[#859F3C] text-white text-xs font-semibold rounded-xl transition-colors cursor-pointer"
                >
                  Search
                </button>
              </div>
            ...
            <div className="mt-3 bg-white/95 backdrop-blur-sm rounded-2xl border border-stone-200/50 shadow-2xl overflow-hidden">
              ...
              <div className="p-4 border-b border-stone-100">
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles className="w-3.5 h-3.5 text-[#859F3C]" />
                ...
                <div className="flex flex-wrap gap-2">
                  {filteredSuggestions.map((term) => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-stone-50 hover:bg-[#859F3C]/15 border border-stone-200/80 hover:border-[#859F3C]/40 text-xs text-stone-700 hover:text-[#47571e] transition-all cursor-pointer"
                    >
                        ...
                        <>
                          {term.split(new RegExp(`(${searchQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi')).map((part, i) =>
                            part.toLowerCase() === searchQuery.toLowerCase()
                              ? <mark key={i} className="bg-[#859F3C]/25 text-[#47571e] font-semibold not-italic rounded">{part}</mark>
                  ...
                  {QUICK_LINKS.map((link) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <ArrowRight className="w-3.5 h-3.5 text-stone-400 group-hover:text-[#859F3C] transition-colors" />
                        <span className="text-sm text-stone-700 group-hover:text-stone-900 transition-colors">{link.label}</span>
                      </div>
                      <span className="text-[10px] uppercase tracking-wider font-bold text-stone-400 group-hover:text-[#859F3C] transition-colors">
                        {link.tag}
                      </span>
                    </Link>
                  ))}
